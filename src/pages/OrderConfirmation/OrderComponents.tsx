...
import { ORDER_STATUS_COLORS, TRACKING_STEPS } from "./constants";
import {
  formatCurrency,
        ...
        {TRACKING_STEPS.map((step, index) => (
          <div key={step.order}>
            <TrackingStep
              ...
              isActive={status === "pending" && step.order === 1}
            />
            ...
            {index < TRACKING_STEPS.length - 1 && (
              <TrackingLine
                isCompleted={
                  ...
                  (step.order === 2 && isOrderPaid(status)) ||
                  (step.order === 3 && isOrderShipped(status))
                }
              />
            )}
          </div>
...
export const OrderPricing = ({ order }: OrderPricingProps) => {
  const { subtotal, shipping, total } = calculateOrderPrices(order);
  ...
  return (
    <div className="space-y-3 pb-8 border-b border-gray-200">
      ...
      <div className="flex justify-between text-gray-600">
        ...
        <p className="font-bold">{shipping === 0 ? "Gratis" : formatCurrency(shipping)}</p>
      </div>

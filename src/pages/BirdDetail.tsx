...
export default function BirdDetail() {
  ...
  const targetCatalogId = useMemo(
    () =>
      Object.keys(BIRD_ITEMS)
        .map(Number)
        .find((catalogId) =>
          BIRD_ITEMS[catalogId].some((item) => item.id === currentItemId),
        ) ?? null,
    [currentItemId],
  );
  const itemsInSameCatalog = useMemo(
    () => (targetCatalogId ? BIRD_ITEMS[targetCatalogId] : []),
    [targetCatalogId],
  );

  const currentItem =
    itemsInSameCatalog.find((item) => item.id === currentItemId) || null;
  const currentCatalog = targetCatalogId
    ? BIRD_CATALOGS.find((c) => c.id === targetCatalogId) || null
    : null;

  const initialIndex = currentItem
    ? itemsInSameCatalog.findIndex((i) => i.id === currentItem.id)
    : 0;

  const [isWheelOpen, setIsWheelOpen] = useState(false);
  const [rotationAngle, setRotationAngle] = useState(
    (initialIndex >= 0 ? initialIndex : 0) * -ANGLE_GAP,
  );
  ...
  const activeIndex = useMemo(() => {
    let nearestIndex = Math.round(rotationAngle / ANGLE_GAP) * -1;
    nearestIndex = Math.max(
      0,
      Math.min(nearestIndex, itemsInSameCatalog.length - 1),
    );
    return nearestIndex;
  }, [rotationAngle, itemsInSameCatalog.length]);

  useEffect(() => {
    const newIndex = itemsInSameCatalog.findIndex(
      (i) => i.id === currentItemId,
    );
    if (newIndex !== -1) {
      const syncedRotation = newIndex * -ANGLE_GAP;
      const frameId = window.requestAnimationFrame(() => {
        setRotationAngle(syncedRotation);
        lastRotation.current = syncedRotation;
      });
  ...
  const handleMovement = useCallback(
    ...
    [itemsInSameCatalog, isMobile, navigate],
  );
  ...
  if (!currentItem) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center h-full bg-plant-light p-10 min-h-screen">
        ...
        <h2 className="text-2xl font-bold text-plant-dark">
          Burung Tidak Ditemukan
        </h2>
        <p className="text-gray-500 mb-6">
          Silakan pilih burung dari menu katalog.
        </p>
        <Link
          to="/"
          className="bg-plant-green text-white px-6 py-3 rounded-xl font-bold"
        >
          Kembali ke Katalog
        </Link>
      </div>
  ...
  return (
    <div className="flex-1 flex min-h-screen bg-plant-light overflow-hidden relative">
      <main className="flex-1 p-6 lg:p-12 transition-all relative z-10 w-full overflow-y-auto h-screen no-scrollbar">
        <div className="max-w-3xl pb-20">
          <div className="mb-4 pt-4 md:pt-0">
            <Link
              to={`/catalog/${currentItem.catalog_id}`}
              className="text-plant-green text-sm font-bold flex items-center gap-2 mb-6 hover:underline"
            >
          ...
          <div className="relative w-full h-64 lg:h-[450px] bg-gradient-to-tr from-plant-green/20 to-transparent rounded-[2.5rem] lg:rounded-[3rem] flex items-center justify-center mb-8 shadow-inner">
            <span className="text-8xl md:text-9xl lg:text-[12rem] transform -scale-x-100 drop-shadow-2xl">
              🦅
            </span>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            <div className="flex-1">
              <h3 className="text-xl md:text-2xl font-bold text-plant-dark mb-2">
                Overview
              </h3>
              <p className="text-gray-600 max-w-md text-sm md:text-base leading-relaxed">
                {currentItem.description}
              </p>
              <p className="text-3xl md:text-4xl font-black text-plant-green mt-4 transition-all">
      ...
      <button
        ...
        className={`absolute top-1/2 right-0 -translate-y-1/2 bg-plant-green text-white w-10 h-24 md:w-14 md:h-32 rounded-l-full shadow-[-4px_0_20px_rgba(13,152,106,0.5)] z-[45] flex items-center justify-center transition-transform duration-500 hover:w-12 md:hover:w-16 ${isWheelOpen ? "translate-x-full" : "translate-x-0"}`}
      >
      ...
      <div
        className={`absolute inset-0 bg-plant-dark/20 backdrop-blur-sm z-40 transition-opacity duration-500 ${isWheelOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}
        onClick={handleOutsideClick}
      />

      <div
        ref={wheelContainerRef}
        className={`absolute top-1/2 rounded-full border-[8px] md:border-[12px] border-plant-green bg-white shadow-[-15px_0_50px_rgba(0,0,0,0.15)] z-50 transition-transform duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)]`}
        style={{
          width: RADIUS * 2,
          height: RADIUS * 2,
          right: RIGHT_OFFSET,
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    transform: `rotate(${-itemAngle - rotationAngle}deg)`,
                  }}
                  onClick={() => {
                ...
                >
                  <div
                    className={`absolute right-[85%] mr-2 bg-white/95 backdrop-blur-md px-4 py-2 md:px-5 md:py-3 rounded-2xl shadow-lg border border-gray-100 whitespace-nowrap transition-all duration-300 origin-right ${isActive ? "opacity-100 scale-100" : "opacity-0 scale-75 pointer-events-none"}`}
                  >
                    <p className="text-[10px] md:text-xs tracking-widest text-gray-400 font-bold mb-1 uppercase">
                      Sertifikat Resmi
                    </p>
                    <p className="font-bold text-plant-dark text-sm md:text-base">
                      {item.certificate}
                    </p>
                    <p className="font-black text-plant-green mt-0.5 text-xs md:text-sm">
                      {(item.price / 1000000).toFixed(1)} Jt
                    </p>
                  </div>
                  <div

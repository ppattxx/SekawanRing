...
export default function BirdDetail() {
  ...
  return (
    <div className="flex-1 flex min-h-screen bg-white overflow-x-hidden relative">
              ...
              <div className="w-full md:w-5/12 flex flex-col justify-center z-20 order-2 md:order-1 mt-8 md:mt-0 pt-0 md:pt-8 lg:pt-16">
                 ...
                 <div className="flex gap-4 pt-2">
                    <button className="flex-1 md:flex-none w-auto md:w-16 h-12 md:h-16 bg-plant-green hover:bg-[#0A875D] transition-colors rounded-[1rem] md:rounded-[1.2rem] flex items-center justify-center text-white shadow-[0_10px_20px_rgba(13,152,106,0.3)]">
                      <svg className="w-5 h-5 md:w-7 md:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
                    ...
                    <button className="w-12 h-12 md:w-16 md:h-16 bg-white hover:bg-gray-50 transition-colors rounded-[1rem] md:rounded-[1.2rem] flex items-center justify-center text-[#0B2F2D] shadow-[0_10px_20px_rgba(0,0,0,0.05)]">
                      <svg className="w-5 h-5 md:w-7 md:h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                    </button>
                 </div>
              </div>
      ...
      <div ref={wheelContainerRef} className={`absolute top-1/2 rounded-full border-[6px] md:border-[12px] border-[#0D986A] bg-white shadow-[-15px_0_50px_rgba(0,0,0,0.15)] z-50 transition-transform duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)]`}
              ...
              {itemsList.map((item, index) => {
                  ...
                  return (
                      <div key={item.id} className="absolute flex items-center justify-center" style={{ transform: `rotate(${itemAngle}deg) translateX(${-RADIUS}px)` }}>
                          ...
                          >
                              <div className={`absolute right-[85%] mr-1 sm:mr-2 bg-white/95 backdrop-blur-md px-3 py-1.5 md:px-5 md:py-3 rounded-xl md:rounded-2xl shadow-lg border border-gray-100 whitespace-nowrap transition-all duration-300 origin-right ${isActive ? 'opacity-100 scale-100' : 'opacity-0 scale-75 pointer-events-none'}`}>

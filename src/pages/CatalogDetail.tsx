...
// --- KONFIGURASI RODA 2D ---
const ANGLE_GAP = 18; // Jarak pasti antar item (dalam derajat). Menjamin jarak selalu sama!
...
export default function CatalogDetail() {
  ...
  // Responsivitas Ukuran Roda
  const isMobile = windowWidth < 768;
  const RADIUS = isMobile ? 280 : 450; 
  // Offset agar roda tidak menutupi seluruh layar (Hanya menjorok 180px di HP, 320px di Desktop)
  ...
  const handleMovement = useCallback((deltaY: number) => {
    ...
    // Auto-Close saat berhenti memutar
  ...
  return (
    <div className="flex min-h-screen bg-plant-light overflow-hidden relative">
        ...
        <div className="p-6">
          ...
          <nav className="space-y-4">
            {BIRD_CATALOGS.map((catalog) => (
              <Link key={catalog.id} to={`/catalog/${catalog.id}`} className={`flex items-center gap-3 p-3 rounded-2xl transition-all ${catalog.id === activeCatalog ? 'bg-plant-green text-white shadow-lg' : 'text-gray-500 hover:bg-plant-light'}`}>
      ...
      {/* Overlay Gelap (Bisa di-klik untuk menutup) */}
      <div 
        className={`fixed inset-0 bg-plant-dark/20 backdrop-blur-sm z-40 transition-opacity duration-500 ${isWheelOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}
        onClick={handleOutsideClick}
      />

      {/* CONTAINER RODA BESAR */}
      <div 
        ...
        style={{
            width: RADIUS * 2,
            height: RADIUS * 2,
            right: RIGHT_OFFSET, // Menjorok ke luar layar agar tidak menutupi seluruh layar HP
            marginTop: -RADIUS, // Pusatkan ke tengah layar
      ...
      >
          ...
          >
              {items.map((item, index) => {
                  ...
                  return (
                      // PENEMPATAN BURUNG DI GARIS LINGKARAN
                      <div 
                          key={item.id}
                          className="absolute flex items-center justify-center"
                          style={{
                              // 1. Putar poros ke sudut item. 2. Dorong item ke ujung kiri (sepanjang radius)
                              // Ini menjamin burung TEPAT di garis border hijau tanpa keluar sedikitpun!
                              transform: `rotate(${itemAngle}deg) translateX(${-RADIUS}px)`,
                          }}
                      >
                          {/* COUNTER-ROTATION: Membuat burung dan teks selalu tegak lurus ke atas! */}
                          <div 
                          ...
                          >
                              {/* Label Putih (Hanya muncul untuk item yang dipilih) */}
                              <div className={`absolute right-[85%] mr-2 bg-white/95 backdrop-blur-md px-4 py-2 md:px-5 md:py-3 rounded-2xl shadow-lg border border-gray-100 whitespace-nowrap transition-all duration-300 origin-right ${
                              ...
                              {/* Ikon Bulat */}
                              <div className={`rounded-full flex items-center justify-center transition-all duration-300 shadow-md ${
                                  isActive 
